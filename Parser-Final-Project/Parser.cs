...
namespace Parser_Final_Project
{
	public class Parser
	{
		...
		private Node StatementSequance()
		{
			...
			/* if (y == current && tokens[current].token_type != Token_Class.SEMICOLON)
			 {
				 return new Node();
			 }*/
			return head;
		...
		private Node Exp()
		{
			...
			/* else if (current < tokens.Count && !IsComparison(tokens[current].token_type) && tokens[current].token_type != Token_Class.CLOSEDBRACKET)
				 return new Node();*/
			if (current < tokens.Count && tokens[current].token_type == Token_Class.CLOSEDBRACKET)
				Match(tokens[current].token_type);
			return node;
		}
		private Node ThenIf()
